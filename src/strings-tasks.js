...
function countVowels(str) {
  ...
  str.split('').forEach((char) => {
    if (wovels.includes(char.toLowerCase())) {
      counter += 1;
    }
  });
  return counter;
...
function isPalindrome(str) {
  const onlyLetters = str
    ...
    .join('');
  const palindrome = onlyLetters.split('').reverse().join('');
  return onlyLetters.toLowerCase() === palindrome.toLowerCase();
}
...
function findLongestWord(sentence) {
  ...
  const invertedWords = sentence.split(' ').reverse();
  invertedWords.forEach((word, index) => {
    if (word.length >= invertedWords[indexOfLongest].length) {
      indexOfLongest = index;
    }
  });
  return invertedWords[indexOfLongest];
}
...
function encodeToRot13(str) {
  function encodeChar(char) {
    ...
    if (index + 1 <= alphabet.length - 13) {
      newIndex = index + 13;
    } else {
      newIndex = 12 - (alphabet.length - 1 - index);
    }
